...
import traceback
from proxy_service.bot_server_http_calls.course \
    import get_bot_token_from_team_id
    ...
    if slack_client:
        while retry_count <= 5:
            ...
            try:
                ...
                traceback.print_exc()
                slack_client = SlackWebClientCacheSingletonManager \
                    .get_cache() \
                    .get_or_add_missing_team_client(team_id=team_id,
                                                    team_token=bot_token)
                retry_count += 1
    else:
        print("Most probably slack app not "
              "configured on system or error creating client.")
