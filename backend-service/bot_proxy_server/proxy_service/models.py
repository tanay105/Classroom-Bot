...
class CommandRequestManager(models.Manager):
    ...
    def create_new_incoming_record(
            self,
            command: str,
            command_parameter: str,
            is_valid_request=True) -> int:
        """
        ...
        req = CommandRequest(
            command=command,
            command_parameters=command_parameter,
    ...
    def update_request(self, request_id: int, request_parameters: dict) -> int:
        ...
        self.filter(
            log_command_request_id=request_id).update(
            **request_parameters)
        return request_id
...
class CommandRequest(models.Model):
    ...
    command_parameters = models.CharField(
